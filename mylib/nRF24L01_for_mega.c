...
#define MOSI_PIN    PORTB2 //PORTB3
#define MISO_PIN    PORTB3 //PORTB4
#define SCK_PIN     PORTB1 //PORTB5
#define SS_PIN      PORTB0 //PORTB2 
#define NRF24_CSN   PORTH5 //PORTB0            /*to enable SPI on nrf24, active LOW*/
#define NRF24_CE    PORTH4 //PORTD7             /*active HIGH, activate chip in RX or TX mode*/
#define IRQ_PIN     PORTH6 //??PORTB1
...
void SPI_Initializer()
{
  ...
  // Inputs
  DDRB &= ~(1 << MISO_PIN); 
  DDRH &= ~(1 << IRQ_PIN);   // for interrupts
  PORTH |= (1 << IRQ_PIN);	 // Turn pullup on
...
{
  DDRH |= (1 << NRF24_CE);  // Set as output
  DDRH |= (1 << NRF24_CSN);

  PORTH |= (1 << NRF24_CSN); // Set bit ---??? not sure if we set or clear
...
void nrf24_SPI(uint8_t input)
{
  if (input == SPI_ON) // if SPI is 0
    PORTH &= ~(1 << NRF24_CSN); // clear
  else
    PORTH |= (1 << NRF24_CSN); // set
}
...
void nrf24_CE(uint8_t input)
{
  if (input == CE_ON) // if CE is 1
    PORTH |= (1 << NRF24_CE); // set
  else
    PORTH &= ~(1 << NRF24_CE); // clear
}
